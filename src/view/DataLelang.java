...
public class DataLelang extends javax.swing.JFrame {
    ...
    private void aturTable(){
        ...
        try{
            sql = "SELECT * FROM tb_lelang INNER JOIN tb_barang ON tb_lelang.id_barang = tb_barang.id_barang LEFT JOIN tb_user ON tb_lelang.id_user = tb_user.id";
            rs = stat.executeQuery(sql);
            while(rs.next()){
                Object[] isi = {rs.getString("id_lelang"),rs.getString("nama_barang"),String.valueOf(fm1.format(rs.getDate("tgl_lelang"))),rs.getString("harga_awal"), rs.getString("harga_akhir") == null ? "Belum Ada" : rs.getString("harga_akhir"), rs.getString("nama") == null ? "Belum Ada" : rs.getString("nama"), rs.getString("nama"), rs.getString("status").substring(0,1).toUpperCase() + rs.getString("status").substring(1)};
                model.addRow(isi);
            }
